...
export function calcularEdad(fechaNacimiento) {
  // console.log("FECHA PAQUI", fechaNacimiento);
  const partes = fechaNacimiento.split("-"); // Separar el string en día, mes y año
  const año = parseInt(partes[0], 10);
  const mes = parseInt(partes[1], 10) - 1; // Restar 1 porque los meses en JS van de 0 a 11
  const dia = parseInt(partes[2], 10);

  const nacimiento = new Date(año, mes, dia);
  const hoy = new Date();
  let edad = hoy.getFullYear() - nacimiento.getFullYear();
  // console.log("EDAD PAUI", edad);
  ...
  if (edad > 100) {
    console.log("ERROR--EDAD MAYOR A 100");
  }
  return edad;
}
export function calcularEdadBarra(fechaNacimiento) {
  const partes = fechaNacimiento.split("/"); // Separar el string en día, mes y año
...
export function AVSCValue() {
  let valuesArray = ["2/10", "3/10", "4/10", "5/10", "6/10"];
  let positionNumber = numeroAlAzar(0, 4);
  return valuesArray[positionNumber];
}

export function AVMCValue() {
  let valuesArray = ["6/10", "7/10", "8/10"];
  let positionNumber = numeroAlAzar(0, 2);
  return valuesArray[positionNumber];
}
